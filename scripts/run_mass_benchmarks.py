"""Mass benchmark evaluation: 10 models x 5 benchmarks.

Runs 10 NIM-hosted LLMs across all 5 benchmarks (TruthfulQA, ARC-Challenge,
WinoGrande, HellaSwag, MMLU) in order from smallest to largest dataset.
    ...
    for attempt in range(3):
        try:
            ...
            if content:
                ...
                # Use argmax of softmax if text parsing failed
                ...
                if first_letter == "?":
                    # Try to parse from text more aggressively
                    for c in text.strip():
...
async def run_benchmark(
    ...
    if not pending:
        # Already complete — compute summary from file
...
async def main() -> None:
    print("=" * 72)
    print("MASS BENCHMARK EVALUATION: 10 models x 5 benchmarks")
    print("=" * 72)
    _init_clients()
