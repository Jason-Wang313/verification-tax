"""Batch A: 2 models x 5 benchmarks via NIM API (aggressive concurrency).

Models: llama-3.3-70b, llama-3.1-8b
Benchmarks: truthfulqa, arc_challenge, winogrande, hellaswag, mmlu (small→large)

Speed settings:
  - PER_MODEL_CONCURRENCY = 64
  - NUM_WORKERS = 96
  - MAX_TOKENS = 5, TIMEOUT = 15s
  - Retry: 3 attempts, 0.5s backoff on 429/500/502/503
  - Flush every 10 items
  - 18 NIM API keys round-robin
"""
...
def build_prompt(benchmark: str, item: dict) -> tuple[str, list[str]]:
    if benchmark == "mmlu":
        ...
        return _build_prompt_mcq(item["question"], choices, letters), letters
        ...
        return _build_prompt_mcq(item["question"], choices, labels), labels
    elif benchmark == "hellaswag":
        return _build_prompt_hellaswag(item["ctx"], item["endings"]), list("ABCD")
    elif benchmark == "winogrande":
        return _build_prompt_winogrande(
...
async def _score_one(
    ...
    for attempt in range(3):
        ...
        try:
            ...
            msg = str(exc).lower()
            if any(t in msg for t in ("429", "rate limit", "500", "502", "503")):
                if attempt < 2:
                    await asyncio.sleep(0.5)
                    continue
            return {
        ...
        while True:
            try:
                ...
                if buffer:
                    ...
                    _maybe_print(counter, start, total, len(completed_ids),
                                 model_name, benchmark, last_print)
            ...
            if len(buffer) >= FLUSH_EVERY:
                ...
                if counter["done"] - last_print >= PRINT_EVERY:
                    _maybe_print(counter, start, total, len(completed_ids),
                                 model_name, benchmark, last_print)
                    last_print = counter["done"]
                buffer = []
...
def _maybe_print(counter, start, total, initial_done, model_name, benchmark, last_print):
    if counter["done"] <= last_print:
        return
    elapsed = (time.time() - start) / 60
    new_done = counter["done"] - initial_done
...
async def main() -> None:
    print("=" * 72)
    print("BATCH A: 2 models x 5 benchmarks (aggressive concurrency)")
    print("=" * 72)
    _init_clients()
